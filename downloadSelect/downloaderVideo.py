...
def downloader(ID, url, extra_args=None, sub_mode="none", sub_args=None, base_dir=None) :
    ...
    if sub_mode == "only":
        return run_yt_dlp(['--write-subs', '--skip-download'] + sub_args + ["-o", download_template] + common + [url])
    sub_flags = (['--write-subs'] + sub_args) if sub_mode == "with" else []
    return run_yt_dlp(['-f', ID, "-o", download_template] + common + extra + sub_flags + [url])
