...
def downloader(ID, url, extra_args=None, sub_mode="none", sub_args=None) :
    ...
    if sub_mode == "only":
        run_yt_dlp(['--write-subs', '--skip-download'] + sub_args + ["-o", download_template] + common + [url])
        return
    sub_flags = (['--write-subs'] + sub_args) if sub_mode == "with" else []
    run_yt_dlp(['-f', ID, "-o", download_template] + common + extra + sub_flags + [url])
